...
@Component({
  selector: 'app-club-file-component',
  standalone: true,
...
export class ClubFiles implements OnInit {
  ...
  loadClubFiles(): void {
    this.clubService.loadClubFiles().subscribe({
      next: (files) => {
  ...
  onFileSelected(event: Event, type: ClubFileType): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      this.clubService.uploadClubFiles([file], type).subscribe({
        next: () => {
          this.loadClubFiles();
        },
  ...
  openPreview(fileId: number, title: string): void {
    ...
    this.clubService.loadClubFileById(fileId).subscribe({
      next: (file) => {
  ...
  triggerEdit(type: ClubFileType): void {
    ...
    input.onchange = (event: Event) => {
      ...
      if (target.files && target.files.length > 0) {
        ...
        this.clubService.updateClubFile(newFile, existingFile.id!).subscribe({
          next: () => {
            this.loadClubFiles();
  ...
  deleteFile(fileId: number): void {
    if (confirm('Are you sure you want to delete this file?')) {
      this.clubService.deleteClubFile(fileId).subscribe({
        next: () => {
          this.loadClubFiles();
          if (this.showPreview) {
            this.closePreview();
